...
import { ConfigManager, CustomConfig } from './ConfigManager';
...
export class ConfigWebview {
    ...
    private constructor(panel: vscode.WebviewPanel, extensionUri: vscode.Uri, configManager: ConfigManager) {
        ...
        const templatePath = path.join(extensionUri.fsPath, 'out', 'strict_template.json');
        this._strictTemplate = fs.readFileSync(fs.existsSync(templatePath) ? templatePath : path.join(extensionUri.fsPath, 'client', 'strict_template.json'), 'utf8');
        try {
    ...
    public static render(extensionUri: vscode.Uri, configManager: ConfigManager, view: 'picker' | 'editor' = 'picker') {
        if (ConfigWebview.currentPanel) {
            ...
            ConfigWebview.currentPanel = new ConfigWebview(panel, extensionUri, configManager);
        }

        if (view === 'editor') {
            ConfigWebview.currentPanel.handleEditConfig(Date.now().toString(), false, true);
        }
    }
    ...
    private handleEditConfig(id: string, isDefault: boolean, isNew: boolean = false) {
        let config: Partial<CustomConfig>;
        if (isDefault) {
            config = { id: 'default', name: 'Strict Default Template', content: this._strictTemplate };
        } else {
        ...
        this._panel.webview.postMessage({ command: 'openEditor', config, isDefault });
    }
    ...
    private async handleSpawn(config: { name: string, content: string }) {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`Failed to spawn config: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    private _getHtmlForWebview(webview: vscode.Webview) {
        const htmlPath = path.join(this._extensionUri.fsPath, 'client', 'webview', 'index.html');
